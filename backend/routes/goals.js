...
// Listar metas — injeta valor do investimento vinculado se houver
router.get('/', async (req, res) => {
  ...
  // Para metas vinculadas a investimento, busca o valor atual investido
  const enriched = await Promise.all(goals.map(async goal => {
    if (!goal.investment_id) return goal;
    const { data: inv } = await supabase
      .from('investments')
      .select('quantity, avg_price, name')
      .eq('id', goal.investment_id)
      .single();
    if (!inv) return goal;
    const investedValue = Number(inv.quantity) * Number(inv.avg_price);
    return { ...goal, investment_current_amount: investedValue, investment_name: inv.name };
  }));
