...
function calcFixedIncome(amount, rate, ratePeriod, startDate, toDate) {
  ...
  const months = Math.max(0, Math.floor((new Date(toDate)-new Date(startDate))/(1000*60*60*24*30.44)));
...
router.get('/evolution', async (req, res) => {
  ...
  res.json({
    ...
    gain_pct: last?.invested>0 ? Math.round(((last.estimated-last.invested)/last.invested)*100*100)/100 : 0,
  });
});
...
// ── Aporte — cria entry + transação marcada como is_investment ────────────
router.post('/:id/entries', async (req, res) => {
  const { quantity, price, date, account_id } = req.body;
  if (!quantity||!price||!date) return res.status(400).json({ error:'Quantidade, preço e data obrigatórios' });

  const supabase  = db(req.token);
  const qty       = Number(quantity);
  const prc       = Number(price);
  const isFixed   = req.body.is_fixed || false;
  const totalCost = isFixed ? prc : qty * prc;

  // Cria entry
  const { data: entry, error } = await supabase.from('investment_entries').insert({
    investment_id: req.params.id, user_id: req.user.id,
    quantity: isFixed ? 1 : qty, price: isFixed ? prc : prc, date,
    account_id: account_id||null,
  }).select().single();
  if (error) return res.status(400).json({ error: error.message });

  // Recalcula qty e preço médio
  ...
  const { data: inv } = await supabase.from('investments').select('type,name,initial_amount').eq('id',req.params.id).single();
  const invIsFixed = ['fixed_income','treasury'].includes(inv?.type);

  const updatePayload = {
    quantity:  invIsFixed ? 1 : Math.round(totalQty*1000000)/1000000,
    avg_price: invIsFixed ? totalVal : Math.round(avgPrice*100)/100,
  };
  if (invIsFixed) updatePayload.initial_amount = Math.round(totalVal*100)/100;
  await supabase.from('investments').update(updatePayload).eq('id',req.params.id);

  // Cria transação is_investment=true (NÃO aparece como despesa no dash)
  // MAS decrementa saldo da conta
  let linkedTx = null;
  if (account_id) {
    // Busca categoria Investimentos
    const { data: cat } = await supabase.from('categories')
      .select('id').eq('user_id',req.user.id).ilike('name','%investimento%').limit(1).single().catch(()=>({data:null}));
    ...
    const { data: tx } = await supabase.from('transactions').insert({
      ...
      status:        'confirmed',
      is_investment: true,  // ← chave: não conta como despesa no dashboard
  ...
  res.status(201).json({
    entry,
    updated_investment: updatePayload,
