    ...
    <span>
      {parts.map((part, i) =>
        part.toLowerCase() === query.toLowerCase()
          ? <mark key={i} style={{background:'var(--amber-dim)',color:'var(--amber)',borderRadius:3,padding:'0 2px',fontWeight:'var(--font-semibold)'}}>{part}</mark>
  ...
  return (
    <>
      <style>{`
        ...
        .search-row:hover { background: var(--bg3) !important; }
      `}</style>
      ...
      <div style={{
        ...
        <div style={{display:'flex',alignItems:'center',gap:'var(--space-3)',padding:'var(--space-3) var(--space-4)',borderBottom:'1px solid var(--border)'}}>
          ...
          {query && (
            <button onClick={()=>setQuery('')}
              style={{width:22,height:22,borderRadius:'var(--radius-full)',border:'none',background:'var(--bg3)',color:'var(--text3)',cursor:'pointer',fontSize:14,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
              ×
            </button>
          )}
        ...
        <div style={{overflowY:'auto',flex:1}}>
          {!query && (
            <div style={{padding:'var(--space-8)',textAlign:'center',color:'var(--text3)'}}>
              ...
              <p style={{fontSize:'var(--text-xs)',marginTop:'var(--space-1)'}}>Mínimo 2 caracteres • Ctrl+K para abrir/fechar</p>
            </div>
          )}
          ...
          {query.length >= 2 && !loading && results.length === 0 && (
            <div style={{padding:'var(--space-8)',textAlign:'center',color:'var(--text3)'}}>
              <div style={{fontSize:36,marginBottom:'var(--space-2)'}}>🤷</div>
              <p style={{fontSize:'var(--text-sm)'}}>Nenhum resultado para "<strong>{query}</strong>"</p>
            </div>
          )}
          ...
          {results.length > 0 && (
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'var(--space-2) var(--space-4)',background:'var(--bg3)',borderBottom:'1px solid var(--border)'}}>
              <span style={{fontSize:'var(--text-xs)',color:'var(--text3)'}}>{results.length} resultado{results.length!==1?'s':''}</span>
          ...
          {Object.entries(grouped).map(([key,group])=>(
            <div key={key}>
              ...
              {group.items.map(tx=>(
                <div key={tx.id} className="search-row"
                  style={{display:'flex',alignItems:'center',gap:'var(--space-3)',padding:'var(--space-3) var(--space-4)',borderBottom:'1px solid var(--border)',transition:'background var(--transition)',background:'var(--bg2)'}}>
                  <div style={{width:7,height:7,borderRadius:'var(--radius-full)',background:tx.categories?.color||'var(--border)',flexShrink:0}}/>
                  <div style={{flex:1,minWidth:0}}>
                    <p style={{fontSize:'var(--text-sm)',color:'var(--text)',fontWeight:'var(--font-medium)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
                      <Highlight text={tx.description||'—'} query={debounced}/>
                    </p>
                    <div style={{display:'flex',gap:'var(--space-2)',marginTop:1}}>
                      ...
                      {tx.categories && (
                        <span style={{fontSize:'var(--text-xs)',color:'var(--text3)'}}>
                          • <Highlight text={tx.categories.name} query={debounced}/>
                        </span>
                      )}
                    </div>
                  </div>
        ...
        <div style={{padding:'var(--space-2) var(--space-4)',borderTop:'1px solid var(--border)'}}>
          <span style={{fontSize:'var(--text-xs)',color:'var(--text3)'}}>
