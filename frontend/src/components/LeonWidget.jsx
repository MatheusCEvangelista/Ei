...
const GREETING = {
  ...
  stressed:'Ei! Vi algumas coisas que merecem atenção ⚠️',
};
...
export default function LeonWidget() {
  ...
  async function handleAction(action) {
    ...
    if (action.type==='confirm_action') { await sendMessage({ message:'sim', question_id:'free' }); }
    if (action.type==='cancel_action')  { await sendMessage({ message:'não', question_id:'free' }); }
  }

  function handleSendFree() { const t=input.trim(); if(!t||loading) return; sendMessage({message:t}); }
  function handleKeyDown(e) { if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();handleSendFree();} }

  function resetChat() {
    setMessages([{from:'leon',text:GREETING[state]||GREETING.neutral}]);
    setInput(''); setShowPreDef(true); setPendingAction(null); setState('asking');
  }
  ...
  const ACTION_STYLES = {
    ...
    danger:  { border:'var(--red)',   bg:'var(--red-dim)',   color:'var(--red)'   },
    default: { border:'var(--indigo)',bg:'var(--indigo-dim)',color:'var(--indigo)' },
  };

  return (
    <>
      <style>{`
        @keyframes leon-pulse{0%,100%{box-shadow:0 0 0 0 ${ls.pulse}55}50%{box-shadow:0 0 0 10px ${ls.pulse}00}}
        @keyframes leon-bounce{0%,100%{transform:translateY(0)}50%{transform:translateY(-6px)}}
        @keyframes tdot{0%,80%,100%{transform:scale(0.4);opacity:.3}40%{transform:scale(1);opacity:1}}
        @keyframes action-in{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:translateY(0)}}
        @keyframes pending-pulse{0%,100%{opacity:1}50%{opacity:.6}}
        .leon-fab{bottom:28px;right:24px;} .leon-panel{bottom:108px;right:24px;width:340px;}
      ...
      <button className="leon-fab" onClick={handleOpen} title="Falar com Leon"
        style={{position:'fixed',zIndex:49,width:68,height:68,borderRadius:'50%',border:'3px solid var(--bg2)',cursor:'pointer',padding:0,overflow:'hidden',background:'transparent',animation:open?'none':'leon-bounce 2.5s ease-in-out infinite, leon-pulse 2s ease-in-out infinite',transition:'transform 0.2s',transform:open?'scale(1.08)':'scale(1)'}}>
      ...
      {open&&(
        <div className="leon-panel fade-up" style={{position:'fixed',zIndex:49,maxHeight:'74vh',background:'var(--bg2)',border:'1px solid var(--border-md)',borderRadius:18,boxShadow:'0 8px 40px rgba(0,0,0,0.3)',display:'flex',flexDirection:'column',overflow:'hidden'}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:10,padding:'12px 14px',borderBottom:'1px solid var(--border)',background:'var(--bg3)',flexShrink:0}}>
            <img src={ls.img} alt="Leon" style={{width:36,height:36,borderRadius:'50%',objectFit:'cover',border:'2px solid var(--border)',flexShrink:0}}/>
            <div style={{flex:1,minWidth:0}}>
              <p style={{fontSize:14,fontWeight:700,color:'var(--text)'}}>Leon</p>
              <p style={{fontSize:11,color:'var(--text3)'}}>Sua IA Financeira 🦎 {pendingAction&&<span style={{color:'var(--amber)',animation:'pending-pulse 1.5s infinite'}}>• aguardando confirmação</span>}</p>
            </div>
            ...
            {messages.map((msg,i)=>(
              <div key={i}>
                <div style={{display:'flex',justifyContent:msg.from==='user'?'flex-end':'flex-start',gap:6,alignItems:'flex-end'}}>
                  {msg.from==='leon'&&<img src={ls.img} alt="Leon" style={{width:26,height:26,borderRadius:'50%',objectFit:'cover',flexShrink:0}}/>}
                ...
                {msg.from==='leon'&&msg.actions?.length>0&&(
                  <div style={{marginLeft:32,marginTop:6,display:'flex',flexWrap:'wrap',gap:5,animation:'action-in 0.3s ease forwards'}}>
                    {msg.actions.map((action,j)=>{
                      const s = ACTION_STYLES[action.variant||'default'];
                      return(
                        <button key={j} onClick={()=>handleAction(action)}
                          style={{padding:'5px 12px',borderRadius:20,fontSize:11,fontWeight:500,border:`1px solid ${s.border}`,background:s.bg,color:s.color,cursor:'pointer',fontFamily:'var(--font)',transition:'all 0.15s',whiteSpace:'nowrap'}}
                          onMouseOver={e=>{e.currentTarget.style.background=s.color;e.currentTarget.style.color='#fff';}}
                          onMouseOut={e=>{e.currentTarget.style.background=s.bg;e.currentTarget.style.color=s.color;}}>
                          {action.label}
                        </button>
                      );
                    })}
          ...
          {!pendingAction&&(
            <div style={{borderTop:'1px solid var(--border)',flexShrink:0}}>
              <button onClick={()=>setShowPreDef(v=>!v)} style={{width:'100%',padding:'7px 12px',background:'var(--bg3)',border:'none',cursor:'pointer',fontSize:11,color:'var(--text3)',fontFamily:'var(--font)',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                <span>💬 Perguntas rápidas</span><span>{showPreDef?'▲':'▼'}</span>
              </button>
              {showPreDef&&(
          ...
          {/* Campo livre */}
          <div style={{display:'flex',gap:6,padding:'8px 10px',background:'var(--bg2)',borderTop:'1px solid var(--border)'}}>
            <input ref={inputRef} type="text" value={input} onChange={e=>setInput(e.target.value)} onKeyDown={handleKeyDown} disabled={loading}
              placeholder={pendingAction?'Confirma? (sim/não)':'Ex: "adiciona R$50 no mercado"...'}
              style={{flex:1,padding:'8px 12px',borderRadius:10,border:`1px solid ${pendingAction?'var(--amber)':'var(--border)'}`,background:'var(--bg3)',color:'var(--text)',fontSize:12,fontFamily:'var(--font)',outline:'none',opacity:loading?0.6:1}}
              onFocus={e=>e.target.style.borderColor=pendingAction?'var(--amber)':'var(--indigo)'}
              onBlur={e=>e.target.style.borderColor=pendingAction?'var(--amber)':'var(--border)'}/>
