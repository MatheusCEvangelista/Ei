...
export default function SummaryCards({ summary, loading }) {
  const cards = [
    ...
    { label: 'Despesas',  value: summary.expense, color: 'var(--red)',   dim: 'var(--red-dim)',   icon: '↓', border: 'rgba(240,94,110,0.15)' },
    {
      ...
      color:  summary.balance >= 0 ? 'var(--indigo)' : 'var(--red)',
      dim:    summary.balance >= 0 ? 'var(--indigo-dim)' : 'var(--red-dim)',
      border: summary.balance >= 0 ? 'rgba(124,127,247,0.15)' : 'rgba(240,94,110,0.15)',
      icon: '=',
    },
  ];
    ...
    <div className="summary-grid fade-up" style={{ display: 'grid', gap: 12 }}>
      {cards.map((card, i) => (
        <div key={card.label} style={{
          background: 'var(--bg2)', border: `1px solid ${card.border}`,
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            ...
            <div style={{
              width: 26, height: 26, borderRadius: 7, background: card.dim,
          ...
          {loading
            ...
            : <p style={{ fontSize: 22, fontWeight: 600, color: card.color, letterSpacing: '-0.03em', fontFamily: 'var(--mono)' }}>
                {fmt(card.value)}
              </p>
          }
        </div>
