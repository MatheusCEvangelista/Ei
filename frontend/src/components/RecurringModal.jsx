...
export default function RecurringModal({item,onClose,onSave}){
  ...
  useEffect(()=>{
    ...
    if(item) setForm({description:item.description||'',amount:item.amount,type:item.type,category_id:item.category_id||'',account_id:item.account_id||'',frequency:item.frequency,day_of_month:item.day_of_month||''});
  },[item]);
        ...
        <form onSubmit={handleSubmit}>
          <div style={{display:'flex',flexDirection:'column',gap:16}}>
            <div style={{display:'flex',background:'var(--bg3)',borderRadius:10,padding:3,border:'1px solid var(--border)'}}>
              {[['expense','Despesa','var(--red)','var(--red-dim)'],['income','Receita','var(--green)','var(--green-dim)']].map(([v,l,c,d])=>(
                <button key={v} type="button" onClick={()=>setForm({...form,type:v})} style={{flex:1,padding:'10px 0',borderRadius:8,fontSize:14,fontWeight:500,border:'none',cursor:'pointer',fontFamily:'var(--font)',transition:'all 0.2s',background:form.type===v?d:'transparent',color:form.type===v?c:'var(--text3)'}}>{l}</button>
              ))}
            </div>
            <div><label style={lbl}>DESCRIÇÃO</label>
              <input type="text" value={form.description} onChange={e=>setForm({...form,description:e.target.value})} placeholder="Ex: Aluguel, Spotify..." style={inp}/>
            </div>
            <div><label style={lbl}>VALOR (R$)</label>
              <input type="number" step="0.01" min="0" required value={form.amount} onChange={e=>setForm({...form,amount:e.target.value})} placeholder="0,00" style={{...inp,fontFamily:'var(--mono)',fontSize:'18px',color:form.type==='income'?'var(--green)':'var(--red)'}}/>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
              ...
              {form.frequency==='monthly'&&(
                <div><label style={lbl}>DIA DO MÊS</label>
                  <input type="number" min="1" max="31" value={form.day_of_month} onChange={e=>setForm({...form,day_of_month:e.target.value})} placeholder="Ex: 5" style={inp}/>
                </div>
              )}
            </div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
