...
export default function EntryModal({ investment, onClose, onSave }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Erro ao registrar aporte');
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div>
                ...
                <input type="number" step="any" min="0" required value={form.quantity}
                  ...
                  placeholder="0" style={{ ...inputStyle, fontFamily: 'var(--mono)' }} />
              </div>
              <div>
                <label style={labelStyle}>PREÇO UNITÁRIO (R$)</label>
                <input type="number" step="0.0001" min="0" required value={form.price}
                  onChange={e => setForm({ ...form, price: e.target.value })}
                  placeholder="0,00" style={{ ...inputStyle, fontFamily: 'var(--mono)', color: 'var(--indigo)' }} />
              </div>
            </div>
