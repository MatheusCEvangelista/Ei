...
export default function AccountModal({ account, onClose, onSave }) {
  ...
  function applyPreset(preset) {
    setForm({ name: preset.name, bank: preset.name, icon: preset.icon, color: preset.color });
  }
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              <div style={{ width: 56, height: 56, borderRadius: 14, background: form.color + '22', border: `1px solid ${form.color}44`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24 }}>
                {form.icon}
              </div>
            </div>
            ...
            {/* Atalhos de banco */}
            <div>
              <label style={labelStyle}>SELECIONE O BANCO</label>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 8 }}>
                {BANK_PRESETS.map(p => (
                  <button key={p.name} type="button" onClick={() => applyPreset(p)} style={{
                    ...
                    border: `1.5px solid ${form.bank === p.name ? p.color : 'var(--border)'}`,
                    background: form.bank === p.name ? p.color + '22' : 'var(--bg3)',
            ...
            <div>
              ...
              <input type="text" required value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Ex: Conta corrente Itaú" style={inputStyle} />
            </div>
