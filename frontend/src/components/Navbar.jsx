  ...
  return (
    <>
      <style>{`
        ...
        .nb-user { display: flex; }
        .nb-ham { display: none; }

        /* Esconde a barra de rolagem no PC mantendo a função de scroll funcional */
        .nb-links::-webkit-scrollbar { display: none; }

        /* MOBILE INTOCADO: ativa o hambúrguer exatamente a partir de 820px */
      ...
      <header style={{background:'var(--bg2)', borderBottom:'1px solid var(--border)', position:'sticky', top:0, zIndex:40}}>
        {/* Aumentado levemente o maxWidth para 1180 para dar respiro aos 11 links no PC */}
        <div style={{maxWidth:1180, margin:'0 auto', padding:'0 16px', height:56, display:'flex', alignItems:'center', justifyContent:'space-between', gap:12}}>
          ...
          {/* NAV DESKTOP AJUSTADA */}
          <nav className="nb-links" style={{
            alignItems: 'center',
            gap: 2,
            flex: 1,
            ...
            {links.map(l => (
              <NavLink key={l.to} to={l.to} end={l.end} style={({isActive}) => ({
                padding: '5px 7px',       /* Discreta redução no padding */
                borderRadius: 7,
                fontSize: 12,             /* Tamanho 12px garante que todos os 11 caibam no PC */
                fontWeight: 500,
                whiteSpace: 'nowrap',
                flexShrink: 0,           /* Impede que o botão encolha/esmague */
                textDecoration: 'none',
          ...
          <div className="nb-user" style={{alignItems:'center', gap:8, flexShrink:0}}>
            <span style={{fontSize:12, color:'var(--text3)', maxWidth:110, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>
              {user?.user_metadata?.name || user?.email}
            </span>
            <NotificationBell />
            <button onClick={toggleTheme} title={isDark ? 'Modo claro' : 'Modo escuro'}
              style={{width:34, height:34, borderRadius:8, border:'1px solid var(--border)', background:'var(--bg3)', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', fontSize:16, flexShrink:0}}>
              {isDark ? '☀️' : '🌙'}
            </button>
            <button onClick={handleLogout}
              style={{fontSize:13, color:'var(--text2)', background:'var(--bg3)', border:'1px solid var(--border)', borderRadius:7, padding:'4px 10px', cursor:'pointer', fontFamily:'var(--font)'}}
        ...
        {open && (
          <div style={{background:'var(--bg2)', borderTop:'1px solid var(--border)', padding:'10px 14px 16px'}} className="fade-up">
            ...
            <div style={{borderTop:'1px solid var(--border)', paddingTop:10, display:'flex', justifyContent:'space-between', alignItems:'center'}}>
              <span style={{fontSize:12, color:'var(--text3)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', maxWidth:'55%'}}>
                {user?.user_metadata?.name || user?.email}
              </span>
              <div style={{display:'flex', gap:8, alignItems:'center'}}>
                <NotificationBell/>
                <button onClick={toggleTheme} style={{width:32, height:32, borderRadius:7, border:'1px solid var(--border)', background:'var(--bg3)', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', fontSize:15}}>
                  {isDark ? '☀️' : '🌙'}
                </button>
