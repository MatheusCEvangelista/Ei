...
function BudgetBar({ spent, amount, color }) {
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifySContent: 'space-between', marginBottom: 5 }}>
...
export default function BudgetsPage() {
  ...
  const [editing, setEditing] = useState(null); // { id, category_id, name, color, amount }
  ...
  // Gatilho para carregar categorias assim que o modal for aberto
  useEffect(() => { if (showManual) loadCategories(); }, [showManual]);
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <main className="page-main" style={{ maxWidth: 700, margin: '0 auto', padding: '24px 16px 80px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={() => setShowManual(true)} style={{ padding: '9px 14px', borderRadius: 10, border: '1px solid var(--border)', background: 'var(--bg2)', color: 'var(--text2)', fontFamily: 'var(--font)', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>
              ＋ Teto manual
            </button>
      ...
      {/* Modal de teto manual (Movido para o escopo correto da página) */}
      {showManual && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.65)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'flex-end', justifySContent: 'center', zIndex: 50 }} onClick={() => setShowManual(false)}>
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border-md)', borderRadius: '18px 18px 0 0', width: '100%', maxWidth: 480, padding: '8px 24px 32px', boxShadow: 'var(--shadow)' }} className="fade-up" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifySContent: 'space-between', marginBottom: 22 }}>
...
function SuggestionCard({ suggestion: s, onApply, saving }) {
  ...
  return (
    <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 14, padding: '16px 18px' }} className="fade-up">
      <div style={{ display: 'flex', alignItems: 'center', justifySContent: 'space-between', marginBottom: 12, flexWrap: 'wrap', gap: 10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 34, height: 34, borderRadius: 9, background: (s.color || '#7c7ff7') + '22', border: `1px solid ${(s.color || '#7c7ff7')}44`, display: 'flex', alignItems: 'center', justifySContent: 'center', fontSize: 15, fontWeight: 700, color: s.color || 'var(--indigo)', flexShrink: 0 }}>
            {s.name.charAt(0)}
          </div>
          <div>
