...
export default function RecurringPage() {
  ...
  async function handleGenerate() {
    setGenerating(true);
    const { data } = await api.post('/api/recurring/generate');
    setGenerated(data.generated);
    setGenerating(false);
  }

  async function handleToggle(item) {
    await api.put(`/api/recurring/${item.id}`, { ...item, active: !item.active });
    load();
  }
      ...
      <main style={{ maxWidth: 640, margin: '0 auto', padding: '24px 14px 80px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24 }}>
          <div>
            ...
            <p style={{ color: 'var(--text3)', fontSize: 13, marginTop: 4 }}>Transações que se repetem automaticamente</p>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={handleGenerate} disabled={generating} style={{
              padding: '9px 14px', borderRadius: 10, border: '1px solid var(--border)',
              background: 'var(--bg2)', color: 'var(--text2)', fontFamily: 'var(--font)', fontSize: 13, fontWeight: 500, cursor: 'pointer',
            }}>
        ...
        {generated !== null && (
          <div style={{ background: generated > 0 ? 'var(--green-dim)' : 'var(--bg3)', border: `1px solid ${generated > 0 ? 'rgba(45,212,160,0.25)' : 'var(--border)'}`, borderRadius: 10, padding: '12px 16px', marginBottom: 16, fontSize: 13, color: generated > 0 ? 'var(--green)' : 'var(--text3)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span>{generated > 0 ? `✓ ${generated} transação(ões) gerada(s) para este mês!` : 'Nenhuma nova transação — tudo já foi gerado neste mês.'}</span>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '60px 0', color: 'var(--text3)' }}>
            ...
            <p style={{ fontSize: 13 }}>Nenhuma transação recorrente cadastrada.</p>
          ...
          <>
            {active.length > 0 && (
              <Section title={`Ativas (${active.length})`}>
                {active.map(item => <RecurringRow key={item.id} item={item} onEdit={() => { setEditing(item); setShowModal(true); }} onDelete={() => handleDelete(item.id)} onToggle={() => handleToggle(item)} />)}
              </Section>
            )}
            {inactive.length > 0 && (
              <Section title={`Pausadas (${inactive.length})`} style={{ marginTop: 14, opacity: 0.7 }}>
                {inactive.map(item => <RecurringRow key={item.id} item={item} onEdit={() => { setEditing(item); setShowModal(true); }} onDelete={() => handleDelete(item.id)} onToggle={() => handleToggle(item)} />)}
              </Section>
            )}
          </>
        )}
  ...
  return (
    <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 14, overflow: 'hidden', ...style }}>
      <div style={{ padding: '10px 16px', borderBottom: '1px solid var(--border)' }}>
        <span style={{ fontSize: 11, color: 'var(--text3)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.07em' }}>{title}</span>
      </div>
      {children}
    </div>
  );
...
function RecurringRow({ item, onEdit, onDelete, onToggle }) {
  const [hovered, setHovered] = useState(false);
  const fmt = v => new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(v);
  const freqLabel = item.frequency === 'weekly' ? 'Semanal' : 'Mensal';
    ...
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{
          ...
          border: `1px solid ${item.type === 'income' ? 'rgba(45,212,160,0.2)' : 'rgba(240,94,110,0.2)'}`,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          color: item.type === 'income' ? 'var(--green)' : 'var(--red)', fontSize: 14, fontWeight: 700,
        }}>🔄</div>
        <div>
          <p style={{ fontSize: 13, fontWeight: 500, color: 'var(--text)' }}>{item.description || '—'}</p>
          <div style={{ display: 'flex', gap: 6, marginTop: 2 }}>
            <span style={{ fontSize: 11, color: 'var(--text3)' }}>{freqLabel}</span>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <span style={{ fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 500, color: item.type === 'income' ? 'var(--green)' : 'var(--red)' }}>
          {item.type === 'income' ? '+' : '-'}{fmt(item.amount)}
        </span>
        {hovered && (
          <div style={{ display: 'flex', gap: 6 }}>
            <button onClick={onToggle} style={{ fontSize: 12, color: item.active ? 'var(--amber)' : 'var(--green)', background: item.active ? 'rgba(245,166,35,0.12)' : 'var(--green-dim)', border: 'none', borderRadius: 6, padding: '4px 10px', cursor: 'pointer', fontFamily: 'var(--font)' }}>
