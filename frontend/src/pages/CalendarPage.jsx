    ...
    <div style={{minHeight:'100vh',background:'var(--bg)'}}>
      ...
      <style>{`
        @media(max-width:820px){
          ...
          .cal-panel  { width: 100% !important; max-height: 300px !important; }
          .cal-weekday-label { display: none; }
          .cal-weekday-short { display: flex !important; }
      ...
      <main className="page-main" style={{maxWidth:900,margin:'0 auto',padding:'24px 16px 80px'}}>
        ...
        {/* Resumo */}
        <div className="summary-grid" style={{gap:10,marginBottom:18}}>
          {[
        ...
        {/* Layout: calendário + painel — FIX: sem style duplicado, flex em vez de grid */}
        <div className="cal-layout" style={{
          display:'flex',
          flexDirection: selected && !isMobile ? 'row' : 'column',
          gap:14,
          alignItems:'flex-start',
        }}>

          {/* ── Calendário ── */}
          <div style={{flex:1,minWidth:0,background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:14,overflow:'hidden'}}>

            {/* Dias da semana — versão desktop */}
            <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',borderBottom:'1px solid var(--border)'}}>
              {WEEKDAYS.map((d,i)=>(
                <div key={d}>
                  <div className="cal-weekday-label" style={{padding:'10px 0',textAlign:'center',fontSize:11,fontWeight:600,color:'var(--text3)',textTransform:'uppercase',letterSpacing:'0.05em'}}>
                    {d}
                  </div>
                  <div className="cal-weekday-short" style={{display:'none',padding:'8px 0',textAlign:'center',fontSize:10,fontWeight:600,color:'var(--text3)'}}>
                    {WEEKDAYS_SHORT[i]}
                  </div>
                </div>
              ))}
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)'}}>
              {cells.map((day,i) => {
                if (!day) return (
                  <div key={`e${i}`} style={{minHeight:cellH,borderRight:'1px solid var(--border)',borderBottom:'1px solid var(--border)',background:'var(--bg3)',opacity:0.4}}/>
                );
                ...
                return (
                  <div key={day}
                    ...
                    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:isMobile?2:4}}>
                      ...
                      {hasUrgent && <span style={{width:5,height:5,borderRadius:'50%',background:'var(--red)',display:'inline-block',flexShrink:0}}/>}
                    </div>

                    {/* Eventos — texto no desktop, bolinhas no mobile */}
                    {isMobile ? (
                      <div style={{display:'flex',flexWrap:'wrap',gap:2,marginTop:2}}>
                        {events.slice(0,4).map((ev,j)=>(
                          <span key={j} style={{width:5,height:5,borderRadius:'50%',background:(TYPE_CONFIG[ev.type]||TYPE_CONFIG.transaction).dot,display:'inline-block'}}/>
                        ))}
                      </div>
                    ) : (
                      <div style={{display:'flex',flexDirection:'column',gap:2}}>
                        {events.slice(0,3).map((ev,j)=>(
                          <div key={j} className="cal-event-label" style={{fontSize:9,color:ev.color||'var(--text3)',background:(TYPE_CONFIG[ev.type]||TYPE_CONFIG.transaction).bg,borderRadius:3,padding:'1px 4px',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',maxWidth:'100%'}}>
                            {ev.label}
                          </div>
                        ))}
          ...
          {/* ── Painel lateral ── */}
          {selected && (
            <div className="cal-panel fade-up" style={{
              ...
              <div style={{flex:1,overflowY:'auto',padding:12,maxHeight: isMobile ? 260 : 'none'}}>
                {selectedEvents.length===0 ? (
                  <p style={{fontSize:13,color:'var(--text3)',textAlign:'center',padding:'24px 0'}}>Nenhum evento neste dia.</p>
                ) : selectedEvents.map((ev,i)=>(
                  <div key={i} style={{padding:'12px 14px',borderRadius:10,marginBottom:8,background:(TYPE_CONFIG[ev.type]||TYPE_CONFIG.transaction).bg,border:`1px solid ${ev.color||'var(--border)'}33`}}>
        ...
        <div style={{display:'flex',gap:12,marginTop:14,flexWrap:'wrap'}}>
          {[
            ...
            {color:'var(--amber)', label:'Parcelas'},
            {color:'var(--indigo)',label:'Recorrentes'},
          ].map(l=>(
