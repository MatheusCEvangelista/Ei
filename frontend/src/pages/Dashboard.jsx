...
import { Card, SkeletonList, EmptyState, Button, SectionLabel } from '../components/ui';
...
function ChangeBadge({ pct }) {
  ...
  return (
    <span style={{
      fontSize:10, fontWeight:'var(--font-semibold)',
      color, background:bg,
...
function InsightCard({ insight, onClick }) {
  ...
  return (
    <div onClick={()=>onClick&&insight.action&&onClick(insight.action.url)}
      style={{background:'var(--bg2)',border:'1px solid var(--border)',borderRadius:'var(--radius-lg)',padding:'var(--space-4)',cursor:insight.action?'pointer':'default',transition:'transform var(--transition), box-shadow var(--transition)',flex:1,minWidth:0}}
      ...
      {insight.action&&<span style={{fontSize:'var(--text-xs)',fontWeight:'var(--font-semibold)',color:'var(--indigo)',background:'var(--indigo-dim)',borderRadius:'var(--radius-sm)',padding:'2px 8px',display:'inline-block'}}>{insight.action.label} →</span>}
    </div>
  );
}
...
function TxRow({ tx, onEdit, onDelete }) {
  ...
  const isInvest = tx.is_investment;
  return (
    <div className="tx-row" style={{display:'flex',alignItems:'center',gap:'var(--space-3)',padding:'var(--space-2) var(--space-1)',borderRadius:'var(--radius-md)',transition:'background var(--transition)'}}
      onMouseEnter={e=>e.currentTarget.style.background='var(--bg3)'}
      onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
      <div style={{width:32,height:32,borderRadius:'var(--radius-sm)',flexShrink:0,background:isInvest?'rgba(124,127,247,0.15)':isIncome?'var(--green-dim)':'var(--red-dim)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,fontWeight:'var(--font-bold)',color:isInvest?'var(--indigo)':isIncome?'var(--green)':'var(--red)'}}>
        {isInvest?'📈':isIncome?'↑':'↓'}
      </div>
      <div style={{flex:1,minWidth:0}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:'var(--space-1)',marginTop:1}}>
          {tx.categories&&<span style={{width:5,height:5,borderRadius:'50%',background:tx.categories.color,display:'inline-block',flexShrink:0}}/>}
          <span style={{fontSize:'var(--text-xs)',color:'var(--text3)'}}>{fmtDate(tx.date)}</span>
          {isInvest&&<span style={{fontSize:9,color:'var(--indigo)',background:'var(--indigo-dim)',borderRadius:3,padding:'0 4px'}}>investimento</span>}
        </div>
      </div>
      <div style={{display:'flex',alignItems:'center',gap:'var(--space-2)',flexShrink:0}}>
        <span style={{fontFamily:'var(--mono)',fontSize:'var(--text-sm)',fontWeight:'var(--font-semibold)',color:isInvest?'var(--indigo)':isIncome?'var(--green)':'var(--red)'}}>
          {isIncome||isInvest?'':'-'}{fmt(tx.amount)}
        </span>
...
export default function Dashboard() {
  ...
  // Escuta FAB salvar transação
  useEffect(()=>{
    function handle() { load(); }
    window.addEventListener('ei:transaction-saved', handle);
    return ()=>window.removeEventListener('ei:transaction-saved', handle);
  },[load]);
  ...
  return (
    <div style={{minHeight:'100vh',background:'var(--bg)'}}>
      ...
      <main className="page-main" style={{maxWidth:860,margin:'0 auto',padding:'var(--space-5) var(--space-4) 80px'}}>
        ...
        {loading ? <SkeletonList n={4} h={90} gap={12}/> : (<>
          ...
          <div style={{background:'linear-gradient(135deg,#7c3aed,#a78bfa)',borderRadius:'var(--radius-xl)',padding:'var(--space-6)',marginBottom:'var(--space-4)',color:'#fff',position:'relative',overflow:'hidden'}}>
            ...
            <div style={{display:'flex',alignItems:'baseline',gap:'var(--space-3)',marginBottom:'var(--space-4)',flexWrap:'wrap'}}>
              <p style={{fontFamily:'var(--mono)',fontSize:'var(--text-3xl)',fontWeight:'var(--font-bold)',letterSpacing:'-0.02em'}}>{fmt(balance)}</p>
              <ChangeBadge pct={changes.balance}/>
            </div>
            <div style={{display:'flex',gap:'var(--space-6)',flexWrap:'wrap'}}>
              {[
                {label:'↑ Receitas',  value:fmt(income),  change:changes.income},
                {label:'↓ Despesas',  value:fmt(expense), change:changes.expense},
                {label:'📈 Investido', value:fmt(invested), change:null},
                {label:'💰 Poupança', value:`${savingRate}%`, change:null},
              ].map(c=>(
                <div key={c.label}>
                  <p style={{fontSize:'var(--text-xs)',color:'rgba(255,255,255,0.7)',marginBottom:2}}>{c.label}</p>
                  <div style={{display:'flex',alignItems:'center',gap:'var(--space-1)',flexWrap:'wrap'}}>
                    <p style={{fontFamily:'var(--mono)',fontSize:'var(--text-md)',fontWeight:'var(--font-semibold)'}}>{c.value}</p>
                    {c.change!==null&&c.change!==undefined&&<ChangeBadge pct={c.change}/>}
                  </div>
                </div>
              ))}
            </div>
          ...
          {insights.length>0 && (
            <div style={{marginBottom:'var(--space-4)'}}>
              ...
              <div style={{display:'flex',gap:'var(--space-3)',flexWrap:'wrap'}}>
                {insights.map((ins,i)=><InsightCard key={i} insight={ins} onClick={url=>navigate(url)}/>)}
              </div>
            </div>
          )}

          {/* Score + Tetos */}
          {(score||budgetAlerts.length>0) && (
            <div style={{display:'grid',gridTemplateColumns:score&&budgetAlerts.length?'1fr 1fr':'1fr',gap:'var(--space-3)',marginBottom:'var(--space-4)'}}>
              {score && (
                <Card>
      ...
      {showTxModal&&<TransactionModal transaction={editingTx} defaultType={txType} onSave={()=>{setShowTxModal(false);load();}} onClose={()=>setShowTxModal(false)}/>}
      {showImport&&<ImportModal onSave={()=>{setShowImport(false);load();}} onClose={()=>setShowImport(false)}/>}
    </div>
  );
}
