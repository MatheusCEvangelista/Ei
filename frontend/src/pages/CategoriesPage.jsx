    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <main style={{ maxWidth: 640, margin: '0 auto', padding: '28px 16px' }}>
        ...
        <Section title={`Padrão (${fixed.length})`} loading={loading}>
          {fixed.map((cat, i) => (
            <CategoryRow key={cat.id} category={cat} i={i}
              onEdit={() => { setEditingCategory(cat); setShowModal(true); }}
              onDelete={() => handleDelete(cat.id)} />
          ))}
        </Section>

        <Section title={`Personalizadas (${custom.length})`} loading={loading} style={{ marginTop: 16 }}>
          {custom.length === 0 && !loading ? (
            <div style={{ textAlign: 'center', padding: '32px 0' }}>
              <p style={{ color: 'var(--text3)', fontSize: 13 }}>Nenhuma categoria personalizada ainda.</p>
              <button onClick={() => setShowModal(true)} style={{
                marginTop: 10, background: 'none', border: 'none', color: 'var(--indigo)',
                fontSize: 13, cursor: 'pointer', fontFamily: 'var(--font)',
              }}>Criar minha primeira categoria →</button>
            </div>
          ) : custom.map((cat, i) => (
            <CategoryRow key={cat.id} category={cat} i={i}
              onEdit={() => { setEditingCategory(cat); setShowModal(true); }}
...
function Section({ title, children, loading, style }) {
  return (
    <div style={{
      ...
      <p style={{ fontSize: 11, color: 'var(--text3)', fontWeight: 600, letterSpacing: '0.07em', textTransform: 'uppercase', marginBottom: 14 }}>
        {title}
      </p>
      {loading
        ? [1,2,3].map(i => <div key={i} className="skeleton" style={{ height: 48, marginBottom: 6 }} />)
        : children
      }
    </div>
  );
...
function CategoryRow({ category, onEdit, onDelete, i }) {
    ...
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{
          ...
          background: category.color + '22', border: `1px solid ${category.color}44`,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          color: category.color, fontSize: 15, fontWeight: 700,
        }}>
        ...
        <div>
          ...
          <p style={{ fontSize: 11, color: 'var(--text3)', marginTop: 1 }}>
            {category.is_fixed ? 'Padrão do sistema' : 'Personalizada'}
          </p>
        </div>
      </div>
      {hovered && !category.is_fixed && (
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={onEdit} style={{
            fontSize: 12, color: 'var(--indigo)', background: 'var(--indigo-dim)',
            border: 'none', borderRadius: 6, padding: '4px 10px', cursor: 'pointer', fontFamily: 'var(--font)',
          }}>Editar</button>
